#!/usr/bin/env python3
"""Atomic checkpoint management for the Fanqie novel production agent."""
...
def cmd_init(args: argparse.Namespace) -> None:
    ...
    data = {
        "schema_version": 2,
        "agent": "fanqie-novel-production-agent",
        "configuration": {
...
def cmd_chapter(args: argparse.Namespace) -> None:
    ...
    chapter = data["chapters"].setdefault(
        key,
        {"number": args.number, "title": args.title, "stage": "pending", "text_file": None, "word_file": None},
    )
